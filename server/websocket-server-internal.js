...
const wss = new WebSocketServer({ port: PORT }, () => {
	console.log(`✅ WebSocket server (internal) running on ws://localhost:${PORT}`);
});
...
function getLogFiles() {
	try {
		...
		console.error('❌ Error reading internal log directory:', err);
		return [];
	}
}
...
wss.on('connection', (ws) => {
	console.log('🔌 Client connected (internal)');
	ws.send(JSON.stringify({ type: 'welcome', message: 'Connected to Mirth log stream (internal)' }));
