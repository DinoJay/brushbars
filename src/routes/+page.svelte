	...
	onMount(async () => {
		try {
			...
			const [devDaysRes, msgDaysRes] = await Promise.all([
				fetch('/mirth-logs/api/devLogs/days?days=30'),
				fetch('/mirth-logs/api/messages/days?days=30')
			]);
			...
			if (!selectedDay) {
				...
				if (days.length > 0) {
					selectedDay = days.reduce(
						(latest, d) => (!latest || (d?.date || '') > latest ? d?.date || latest : latest),
						''
					);
				}
			}
